...
<body>
    ...
    <div class="header">
        <div style="display:flex; align-items:center; gap:10px;">
            <img src="{{ public_path('images/prysmian-logo.png') }}" alt="Prysmian" style="height:40px;">
            <div>
                <div class="title">Production Report</div>
                <div class="meta">Generated: {{ now('Asia/Jakarta')->format('Y-m-d H:i') }} WIB</div>
            </div>
        </div>
        <div class="meta" style="text-align:right;">
            Machine: <b>{{ $h->machine }}</b><br>
            Crosshead: <b>{{ $h->crosshead_type }}</b><br>
            Dies Type: <b>{{ $h->dies_type }}</b>
        </div>
    </div>
    ...
    <div class="card grid">
        <div>
            <table>
                ...
                <tr>
                    <td>Material</td>
                    <td>{{ optional($h->material)->name ?? '-' }}</td>
                </tr>
                <tr>
                    <td>Production Date</td>
                    <td>{{ optional($h->production_date)->format('Y-m-d') }}</td>
                </tr>
                <tr>
                    <td>Creator</td>
                    <td>{{ $h->creator }}</td>
        ...
        <div>
            <table>
                ...
                <tr>
                    <td>Input Ø</td>
                    <td>{{ $fmt($h->input_diameter) }} mm</td>
                </tr>
                <tr>
                    <td>Working Thickness</td>
                    <td>{{ $fmt($h->working_thickness) }} mm</td>
                </tr>
                <tr>
                    <td>Output Ø</td>
                    <td>{{ $fmt($h->output_diameter) }} mm</td>
                </tr>
                <tr>
                    <td>DDR</td>
                    <td>{{ $fmt($h->ddr, 3) }}</td>
    ...
    <div class="card">
        <table>
            ...
            <tr>
                <td>
                    {!! $coreText !!}
                    @if($h->core_die_id)<div class="mt8"><span class="badge">stock</span></div>@endif
                </td>
                <td>
                    {!! $ringText !!}
                    @if($h->ring_die_id)<div class="mt8"><span class="badge">stock</span></div>@endif
                </td>
                <td class="right small muted">
                    Report ID: #{{ $h->id }}<br>
                    Created: {{ optional($h->created_at)->format('Y-m-d H:i') }}
                </td>
            </tr>
        </table>
    </div>
