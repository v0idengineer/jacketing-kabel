...
<div x-data="{ show: false }" x-init="setTimeout(() => show = true, 100)">
    <div
        ...
        <form id="jacketingForm" method="POST" action="{{ route('jacketing.store') }}" x-data="{ step: 1 }">
            @if ($errors->any())
            ...
            {{-- penting: supaya kolom machine tidak NULL --}}
            <input type="hidden" name="machine" value="{{ $machine }}">

            {{-- action untuk tombol (pakai _action, jangan "action") --}}
            <input type="hidden" id="form_action" name="_action" value="">

            {{-- state pilihan aktif (campuran calc/stock) --}}
            ...
            {{-- id stok jika dipilih dari stock --}}
            ...
            <div id="card1" class="mb-6 rounded-2xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition-all">
                ...
                <div class="p-6 space-y-5">
                 ...
                 hover:border-slate-400">
                                ...
                                @foreach($crossheads as $item)
                                <option
                                    value="{{ $item }}"
                                    {{ old('crosshead_type', $form1['crosshead_type'] ?? '') == $item ? 'selected' : '' }}>
                                    {{ $item }}
                                </option>
                                @endforeach
                            </select>
                 ...
                 hover:border-slate-400">
                                <option value="">-- Pilih --</option>
                                @foreach($diesTypes as $item)
                                <option
                                    value="{{ $item }}"
                                    {{ old('dies_type', $form1['dies_type'] ?? '') == $item ? 'selected' : '' }}>
                                    {{ $item }}
                                </option>
                                @endforeach
                            </select>
                ...
                <div class="p-6 space-y-5"
                    ...
                    <div>
                        ...
                        <input type="text" name="creator"
                            value="{{ old('creator', $form2['creator'] ?? '') }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="cable_type"
                            value="{{ old('cable_type', $form2['cable_type'] ?? '') }}"
                            x-bind:readonly="step >= 3"
                            x-bind:class="step >= 3 ? 'opacity-60' : ''"
                            class="block w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-slate-700 shadow-sm
                    focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"
                    ...
                    <div>
                        ...
                        <input type="text" name="sap_number"
                            value="{{ old('sap_number', $form2['sap_number'] ?? '') }}"
                            x-bind:readonly="step >= 3"
                            x-bind:class="step >= 3 ? 'opacity-60' : ''"
                            class="block w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-slate-700 shadow-sm
                    focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"
                    ...
                    <div>
                        ...
                        <input type="text" name="order_by"
                            value="{{ old('order_by', $form2['order_by'] ?? '') }}"
                            x-bind:readonly="step >= 3"
                            x-bind:class="step >= 3 ? 'opacity-60' : ''"
                            class="block w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-slate-700 shadow-sm
                    focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"
                    ...
                    <div>
                       ...
                       hover:border-slate-400"
                                ...
                                <option value="{{ $proc }}" {{ old('process', $form2['process'] ?? '') == $proc ? 'selected' : '' }}>
                                    {{ ucwords(str_replace('_', ' ', $proc)) }}
                                </option>
                                @endforeach
                            </select>
                            <svg class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-500"
                    ...
                    focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"
                            value="{{ old('date', $jakartaDate) }}" readonly>
                    </div>
                    ...
                    <div>
                       ...
                       hover:border-slate-400"
                                ...
                                <optgroup label="Tersedia">
                                    @foreach($avail as $m)
                                    <option value="{{ $m->id }}" data-has-stock="1"
                                        style="color:#16a34a; font-weight:600;"
                                        {{ $selected === (string)$m->id ? 'selected' : '' }}>
                                        {{ $m->name }} (stok: {{ (int)$m->stock_count }})
                                    </option>
                                    @endforeach
                                </optgroup>
                                @endif
                                ...
                                <optgroup label="Tidak ada stok">
                                    @foreach($empty as $m)
                                    <option value="{{ $m->id }}" data-has-stock="0" disabled style="color:#94a3b8;">
                                        {{ $m->name }} (kosong)
                                    </option>
                                    @endforeach
                                </optgroup>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="1" max="50"
                            ...
                            value="{{ old('input_diameter', $form2['input_diameter'] ?? '') }}"
                            x-bind:readonly="step >= 3"
                            x-bind:class="step >= 3 ? 'opacity-60' : ''"
                            class="block w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-slate-700 shadow-sm
                    focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0.5" max="5"
                            ...
                            value="{{ old('working_thickness', $form2['working_thickness'] ?? '') }}"
                            x-bind:readonly="step >= 3"
                            x-bind:class="step >= 3 ? 'opacity-60' : ''"
                            class="block w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-slate-700 shadow-sm
                    focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="output_diameter" id="output_diameter"
                            value="{{ old('output_diameter', $form2['output_diameter'] ?? '') }}"
                            class="block w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-700 shadow-sm
                    focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"
            ...
            <div x-data="{open:true}" x-show="open" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
                <div class="bg-white rounded-2xl p-6 w-[420px] shadow-xl">
                    ...
                    <div class="flex gap-3 justify-end">
                        <a href="{{ route('design.print', ['recipe'=>session('saved_recipe_id')]) }}"
...
{{-- Recommender + Target Core/Ring + DDR/DBR (Card 3) --}}
<script>
    function num(v) {
    ...
    async function loadRecommendations() {
        ...
        const url = new URL(@json(route('dies.recommend')));
        ...
        btnReport?.addEventListener('click', async (e) => {
            ...
            try {
                ...
                if (ct.includes('application/pdf') || ct.includes('application/octet-stream')) {
                    ...
                    setTimeout(() => {
                        window.location.assign(@json(route('history')));
                    }, 1500);
                    return;
                }
